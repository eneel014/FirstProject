<?php if (isset($_POST['btnSubmitEditClient'])): ?>
	...
	if (isset($_POST['txtRereferredBy'])) {
		$referredBy = htmlspecialchars($_POST['txtRereferredBy'],ENT_QUOTES);
	}
...
<?php 
	...
	$query = "SELECT * FROM tbl_clients	 WHERE user_id = '$user_id' AND id != '$thisID'";
	...
	if($user_id != '' && $query->num_rows > 0) {
		...
		$query = "UPDATE `tbl_clients` SET `account_update_status` = '1' WHERE id = '$thisID'";
		$query = $conn->query($query);
		$empID = $_COOKIE['empID'];
		$empType = $_COOKIE['cooUserType'];

		// $query1 = "SELECT * FROM `tbl_clients_dummy` WHERE client_id = '$thisID'";
		...
		if ($empType != 3) {
			...
			if($queryCheck->num_rows > 0) {
				...
				`dateUpdated` = 'NOW()',
				`corpo_tin` = '$corpoTIN',
				`corpo_sss` = '$corpoSSS' WHERE client_id = '$thisID'";
				$toConfirm = 1;
			} else {
				$query = "INSERT INTO `tbl_clients_temp`(
					`client_id`,
					`user_id`,
					`initial_password`,
					`status`,
		...
		if ($thisResultEdit = $conn->query($query) === TRUE) {
			...
			$editLogQuery = "SELECT * FROM `tbl_clients` WHERE `id` = '$id'";
			$resultLogQuery = $conn->query($editLogQuery);
			$resultLQ = $resultLogQuery->fetch_assoc();
			$firstname = $resultLQ['firstname'].' '.$resultLQ['lastname'];
			insertAuditLog("$logName", "UPDATED $firstname Detail", $conn);
			?>
			<?php if (isset($toConfirm)): ?>
				<script type="text/javascript">
					window.location.href = "client-details?id=<?=$id?>&toConfirm";
				</script>
			<?php else: ?>
				<script type="text/javascript">
					window.location.href = "client-details?id=<?=$id?>&updated";
				</script>
			<?php endif ?>
			<?php
		} else {
